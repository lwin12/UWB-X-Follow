...
const float COM8_X = -0.20;
const float COM8_Y =  0.00;
const float COM9_X =  0.20;
const float COM9_Y =  0.00;
const float COM11_X =  0.00;
const float COM11_Y = -0.45;
...
void setup() {
  ...
  delay(500); // let modules boot

  SerialUWB1.println("acts 0 0 0 1 0 1 1 1 0");
  SerialUWB2.println("acts 0 0 0 1 0 1 1 1 0");
  SerialUWB3.println("acts 0 0 0 1 0 1 1 1 0");

  delay(1000); // Give modules time to apply the command
  Serial.println("UWB mode set with acts command.");

  delay(500); // Allow acts mode to settle

  SerialUWB1.println("les");
  SerialUWB2.println("les");
  SerialUWB3.println("les");
...
/* // NOW SWITCHING TO A SEND ONCE AT START OPERATION
  // Send periodic requests if none are updated
  if (!d1_updated && !d2_updated && !d3_updated && millis() - lastRequestTime >= requestInterval) {
  requestDistances();
...
String getActionFromHeadingAndDistance(float heading, float distance) {
  ...
  if (distance > 2.0) return "forward";
  if (distance < 0.5) return "backward";
  return "hold";
}
